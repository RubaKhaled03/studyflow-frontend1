...
export function getCourseDetails(courseId: string): Course | null {
  // First, try to get from localStorage (user-created or modified courses)
  const storedCourses = getCoursesFromStorage();
  const storedCourse = storedCourses.find((c) => c.id === courseId);
...
// Initialize SEED_COURSES with all detailed mock courses - must be before export
function initializeSeedCourses(): Course[] {
...
export const SEED_COURSES: Course[] = initializeSeedCourses();
