...
export default function AcademicPlanningPage() { // Changed component name
  const { state, isLoaded, addSemester, updateSemester, deleteSemester, addCourse, updateCourse, deleteCourse, updateState } = useAppState(); // Updated useAppState destructuring
  ...
  const [isResetPlannerDialogOpen, setIsResetPlannerDialogOpen] = useState(false);
  ...
  const totalRequiredCredits = parseInt(state.userProfile.totalCreditHours) || state.academicPlanning.config.totalRequiredCredits || 144; // Corrected typo and used state.academicPlanning.config

  // Derived calculations with baseline
  const cumulativeAverage = calculateCumulativeAverage(courses, baselineAverage, baselineCredits);
  const passedCredits = calculatePassedCompletedCredits(courses, baselineCredits);
  const plannedCredits = calculatePlannedCredits(courses);
  const inProgressCredits = calculateInProgressCredits(courses);

  // Prior courses logic
  const priorCourses = courses.filter(c => c.semesterId === "prior-completed");
  ...
  // --- Handlers ---
  const handleOpenAddSemester = () => { // Changed handler name
    setEditingSemester(null);
    setIsSemesterDialogOpen(true); // Changed state variable name
  };

  const handleOpenEditSemester = (semester: PlannerSemester) => { // Changed handler name
  ...
  const confirmDeleteSemester = () => {
    if (semesterToDelete) {
      ...
      courses.filter(c => c.semesterId === semesterToDelete).forEach(c => deleteCourse(c.id));
      setSemesterToDelete(null);
    }
  };

  const handleOpenAddCourse = (semesterId: string) => { // Changed handler name
  ...
  const handleSaveCourse = (courseData: Omit<Course, "id">) => { // Changed parameter type
    // Validation for prior courses
    if (activeSemesterId === "prior-completed") {
      const currentCredits = priorCourses.reduce((sum, c) => sum + c.credits, 0);
      const isUpdating = !!editingCourse;
      const creditDiff = isUpdating ? courseData.credits - (editingCourse?.credits || 0) : courseData.credits;

      if (currentCredits + creditDiff > baselineCredits) {
        toast.error(`Total prior credits cannot exceed your baseline of ${baselineCredits} hours.`);
        return;
      }
    }
    ...
    if (editingCourse) {
      ...
      addCourse({ ...courseData, id: crypto.randomUUID(), semesterId: activeSemesterId || "" } as Course);
    }
    setEditingCourse(null);
    setActiveSemesterId(null);
  };

  const validatePriorCourseAdd = (credits: number) => {
    if (activeSemesterId === "prior-completed" && totalPriorCredits + credits > baselineCredits) {
      return false;
    }
    return true;
  };
...
<div className="space-y-6 pb-8 animate-in fade-in zoom-in-95 duration-500">      
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-card p-6 md:p-8 rounded-2xl border shadow-sm">
        <div className="space-y-1 max-w-2xl">
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Academic Progress</h1>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <Button variant="outline" size="sm" className="hidden sm:flex text-muted-foreground hover:text-foreground">
             <Settings2 className="mr-2 h-4 w-4" /> Required Hours: {totalRequiredCredits}
          </Button>
          ...
          <Button onClick={handleOpenAddSemester} className="w-full sm:w-auto shadow-sm"> {/* Changed handler name */}
      ...
      <div className={cn(
          ...
          <section className="xl:col-span-4 space-y-4 xl:sticky xl:top-8 animate-in slide-in-from-left-4 duration-700">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-bold tracking-tight text-foreground">Prior Courses</h2>
                <Badge variant="outline" className="bg-blue-50 text-blue-600 border-blue-100">History</Badge>
              </div>
            ...
            <div className="bg-slate-50/50 dark:bg-slate-900/20 border border-border/50 rounded-2xl p-4">
              <div className="flex flex-col gap-3">
                 {priorCourses.map(course => (
                   <div key={course.id} className="bg-card border rounded-xl p-4 flex items-center justify-between group hover:shadow-sm transition-shadow">
                     <div className="flex flex-col gap-1 overflow-hidden">
                       <span className="font-semibold text-sm truncate">{course.title}</span>
                       <div className="flex items-center gap-2">
                          <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">{course.credits} Cr</span>
                          <span className="w-1 h-1 rounded-full bg-muted-foreground/30" />
                          <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-1.5 py-0.5 rounded-md">{course.numericGrade}%</span>
                       </div>
                     </div>
                     <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                       <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleOpenEditCourse(course)}>
                         <Edit2 className="h-3.5 w-3.5" />
                       </Button>
                       <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500 hover:text-red-600 hover:bg-red-50" onClick={() => handleDeleteCourse(course.id)}>
                 ...
                 >
                   ...
                   <span className="text-sm font-semibold">Add Prior Course</span>
                 </Button>
              </div>
            </div>
          </section>
        ...
        <section className={cn(
          ...
          showPriorSection ? "xl:col-span-8" : "xl:col-span-12 lg:max-w-5xl lg:mx-auto w-full"
        )}>
          <div className="flex items-center justify-between">
          ...
          {semesters.filter(s => s.id !== "prior-completed").length === 0 ? (
            ...
            <div className={cn(
              "grid gap-6",
              showPriorSection ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1 md:grid-cols-2"
            )}>
              {semesters
                ...
                <SemesterCard
                  key={semester.id}
                  semester={semester}
                  courses={courses.filter(c => c.semesterId === semester.id)} // Filter courses by semesterId
