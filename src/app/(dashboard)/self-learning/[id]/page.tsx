...
import { LearningPlan, LearningStage, LearningMilestone, LearningResource, SelfLearningTask } from "@/types/self-learning";
import { StageTasksList } from "@/components/self-learning/stage-tasks";
import { loadPlans, savePlans, calcPlanProgress, formatDate } from "@/lib/self-learning/utils";
...
export default function PlanDetailsPage() {
  ...
  const handleSaveStage = (stage: LearningStage) => {
    const updated = { ...plan, stages: plan.stages.some(s => s.id === stage.id)
  ...
  const confirmDeleteStage = () => {
    ...
    persistPlan({ ...plan, stages: plan.stages.filter(s => s.id !== stageToDelete), updatedAt: new Date().toISOString() });
    setStageToDelete(null);
  };

  const handleCompleteStage = (id: string) => {
    const stages = plan.stages.map(s => s.id === id ? { ...s, status: "completed" as const, updatedAt: new Date().toISOString() } : s);
    persistPlan({ ...plan, stages, updatedAt: new Date().toISOString() });
  };

  // Milestone handlers
  const handleSaveMilestone = (milestone: LearningMilestone) => {
    const updated = { ...plan, milestones: plan.milestones.some(m => m.id === milestone.id)
  ...
  const handleToggleMilestone = (id: string) => {
    const milestones = plan.milestones.map(m => m.id === id ? { ...m, completed: !m.completed } : m);
  ...
  const confirmDeleteMilestone = () => {
    ...
    persistPlan({ ...plan, milestones: plan.milestones.filter(m => m.id !== milestoneToDelete), updatedAt: new Date().toISOString() });
  ...
  return (
    <div className="space-y-6 pb-8 animate-in fade-in duration-300">
      ...
      <div className="space-y-4">
        <Button variant="outline" onClick={() => router.back()} className="gap-2 w-fit rounded-xl">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
              <div className="space-y-2 flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge className={cn("gap-1 text-xs", badge.className)}>{badge.icon}{badge.label}</Badge>
                </div>
                {plan.goal && <p className="text-muted-foreground text-sm">{plan.goal}</p>}
                <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                  {plan.category && <span className="px-2 py-0.5 rounded-full bg-violet-100 dark:bg-violet-900/20 text-violet-700 dark:text-violet-400 font-medium">{plan.category}</span>}
                  {plan.targetSkill && <span className="flex items-center gap-1"><Target className="w-3 h-3" />{plan.targetSkill}</span>}
                  <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />Started {formatDate(plan.startDate)}</span>
                  {plan.endDate && <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />Ends {formatDate(plan.endDate)}</span>}
                </div>
              </div>
      ...
      <Card className="border-border/60 shadow-sm">
        <CardContent className="pt-5 space-y-2">
          ...
          {progress === 100 && <p className="text-sm text-emerald-600 dark:text-emerald-400 font-medium flex items-center gap-1"><CheckCircle2 className="w-4 h-4" /> All stages completed! Amazing work.</p>}
        </CardContent>
      </Card>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {plan.stages.length === 0 ? (
            <div className="py-12 text-center border-2 border-dashed border-border/50 rounded-2xl text-muted-foreground bg-muted/10">
              ...
              <Button size="sm" variant="ghost" onClick={() => setStageDialogOpen(true)} className="mt-2 gap-1"><Plus className="h-3.5 w-3.5" />Add First Stage</Button>
            </div>
          ) : (
                    ...
                    <div className="px-5 py-4 flex items-center gap-3 hover:bg-muted/40 transition-colors">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        {stage.status !== "completed" && (
                          <Button size="sm" variant="ghost" className="text-xs gap-1 h-7 px-2 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20"
                    ...
                    {isExpanded && (
                      <div className="border-t border-border/50 px-5 py-4 space-y-4 bg-muted/20">
                        ...
                        <div className="pb-2">
                          <StageTasksList 
                            tasks={stage.tasks || []} 
                            onTasksChange={(newTasks) => handleStageTasksChange(stage.id, newTasks)}
                          />
                        </div>
                        ...
                        {!stage.description && !stage.goals && !stage.tasks?.length && <p className="text-sm text-muted-foreground italic">No details or tasks added yet.</p>}
                      </div>
                    )}
                  </Card>
                );
          ...
          <div className="flex items-center justify-between mt-6">
            <h2 className="text-xl font-bold text-foreground flex items-center gap-2"><Flag className="h-5 w-5 text-amber-500" />Milestones</h2>
          ...
          {plan.milestones.length === 0 ? (
            <div className="py-10 text-center border-2 border-dashed border-border/50 rounded-2xl text-muted-foreground bg-muted/10">
              ...
              <Button size="sm" variant="ghost" onClick={() => setMilestoneDialogOpen(true)} className="mt-2 gap-1"><Plus className="h-3.5 w-3.5" />Add Milestone</Button>
            </div>
          ) : (
            <div className="space-y-2">
              {plan.milestones.map(m => (
                <div key={m.id} className={cn("flex items-center gap-3 p-4 rounded-xl border transition-all group",
                  m.completed ? "bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-200/50 dark:border-emerald-800/30 opacity-75" : "bg-card border-border/60 hover:border-primary/20")}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {m.targetDate && <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5 flex items-center gap-1"><Calendar className="w-3 h-3" />{formatDate(m.targetDate)}</p>}
                ...
                <div className="relative">
                  ...
                  {plan.stages.length > 0 ? (
                    <p className="text-xs text-muted-foreground">
                       {plan.stages.filter(s => s.status !== "completed").length} stage(s) remaining
                    </p>
                  ) : (
                    <p className="text-xs text-muted-foreground">Add stages to map out your journey.</p>
                  )}
                  {plan.milestones.filter(m => !m.completed && m.targetDate).sort((a, b) =>
      ...
      <StageFormDialog open={stageDialogOpen} onOpenChange={setStageDialogOpen} onSave={handleSaveStage}
        initialData={editingStage} planId={planId} nextOrder={plan.stages.length + 1} />
      <MilestoneFormDialog open={milestoneDialogOpen} onOpenChange={setMilestoneDialogOpen} onSave={handleSaveMilestone}
        initialData={editingMilestone} planId={planId} />
